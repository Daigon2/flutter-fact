...
damit nicht fertig**, sondern wie Schritt 49 aufgeteilt: die Kapitelliste
steht, der Lesemodus ist der Rest. Eine Kapitelkarte ist deshalb tippbar
und ohne Ziel.

Der Reiseführer hat jetzt drei Zustände in einem Reiter, genau wie der
Kopf von `screen-wallet.jsx` sie aufzählt: Regal, Buchdeckel,
Inhaltsverzeichnis. Der Bildschirm hält dafür **zwei** Felder,
`_openCityKey` und `_view`, und das ist kein Zierrat: der Weg vom
Inhaltsverzeichnis zurück führt auf den Deckel **derselben** Stadt, und
mit einem Feld müsste die Stadt mitgeschleppt werden. Ein Test hält genau
diesen Weg fest.

#### Zwei Unterschiede, die gleich aussehen und keine sind

* **Ein Kapitel, für das die Stadt keine Fakten hat, fehlt ganz**
  (`.filter(ch => ch.allCatFacts.length > 0)`).
* **Ein Kapitel ohne gesammelten Fakt steht da**, auf 45 Prozent
  Deckkraft, trägt „noch nicht entdeckt" statt seines
  Namens und nimmt keinen Tipp an.

Der Kopf der Quelldatei nennt das Prinzip dahinter: „Lücken-Counter
auf jeder Ebene". Ein Kapitel, das man nicht sieht, weckt keinen
Ehrgeiz; eines, das es in dieser Stadt nicht gibt, wäre eine Lüge.

#### Zwei Zahlen, die nach Fehlern aussehen

* **Die römische Zahl zählt über alle sechs Kapitel, nicht über die
  sichtbaren.** `wltToRoman(idx + 1)` mit `idx` **vor** dem Filter. Eine
  Stadt, die nur Mythos-Fakten hat, zeigt ein einzelnes Kapitel
  „III". Das ist die Kennung des Kapitels im Band und keine
  laufende Nummer der Liste.
* **Die Seitenzahl läuft über die gesammelten Fakten, nicht über alle.**
  Ein Kapitel ohne Gesammeltes verbraucht keine Seite, das nächste fängt
  auf derselben an. Die Buchmetapher zu Ende gedacht: das Buch enthält,
  was man gelesen hat, und wächst mit dem Sammeln.

Beide sind mit einer Mutation festgenagelt, weil ein späterer „Fix" sie
sonst lautlos verschöbe.

#### Was beim Bauen überrascht hat

* **Die Kategorietabelle ist jetzt erzeugt, und die Reihenfolge stand
  vorher von Hand da.** `libraryCategoryOrder` war eine abgeschriebene
  Liste aus Schritt 46; bei einer siebten Kategorie wäre sie stumm
  veraltet. Sie kommt jetzt aus `wallet-colors.jsx`, und das Werkzeug
  bricht ab, wenn Tabelle und Reihenfolge Löcher bekommen. Vierter
  Eintrag der Registrierung, dritter auf derselben Quelldatei.
* **Der Schlüssel steht in der Quelle zweimal**, als Kartenschlüssel und
  als Feld `key` im Objekt. Das Werkzeug prüft jetzt, dass beide gleich
  sind: liefen sie auseinander, könnte niemand sagen, welche Seite falsch
  ist.
* **Der Rückfall `t('cat.' + key) || c.kategorie` ist wieder toter Code**,
  wie bei E-63 und E-73, **richtet hier aber keinen Schaden an**: alle
  sechs Schlüssel existieren in beiden Sprachen. Nachgezählt statt
  behauptet, mit einem Test über alle sechs.
* **Und dabei fiel das Argument für E-78 auf:** die Namen für drei der
  fünf zusammengefalteten Kategorien liegen schon übersetzt vor. Die
  Inhaltsentscheidung kostet damit höchstens zwei Wortlaute.
...
drei Zustände, die der Kopf von `screen-wallet.jsx` aufzählt: **Bibliothek**,
also Kopfkarte, Bücherregal und Trophäenzeile. Stadt-Band (Schritt 46) sowie
Kapitel und Lesemodus (Schritt 47) folgen; ein Buchrücken ist deshalb heute
tippbar und ohne Ziel, dieselbe Bauform wie die Fakt-Akte in Schritt 21.

**Der Bildschirm gehört `collection` und nicht `library`.** Das war am
31.08.2026 entschieden (`lib/features/README.md`, ADR-006) und ist beim Bauen
nicht wieder aufgemacht worden.

#### Die Abweichung, die den Schritt geprägt hat

`wltCityKey` fragt `detectCity(lat, lng)`, und das ist eine
Nächster-Nachbar-Suche über zwölf Stadtmitten **ohne Entfernungsgrenze**. Ein
Fakt in Paris landet bei Heidelberg, `f.ort` wird nie erreicht, und die
Stadt-Trophäen kommen aus einer anderen Quelle als die Bände. Gruppiert wird
deshalb nach `FactCity.slug`, der Spalte in der Normalisierung von
`public._slugify`. Alles dazu in E-75 und im Kopf von `library_city_key.dart`.

**Wichtig für die Zukunft:** damit entsteht **keine** sechste
Stadtschlüssel-Variante. Genau das ist die Falle von E-56, und `FactCity.slug`
liegt seit Schritt 4 im Repository. Wer hier eine eigene Normalisierung
schreibt, macht E-56 schlimmer.

#### Vier weitere Funde derselben Datei

* **E-73**, dreißig von sechsunddreißig Trophäen zeigen ihren rohen Schlüssel.
* **E-74**, die Schlagzeile zeigt das Doppelte der gesammelten Fakten.
* **E-76**, 410 Zeilen toter Code, darunter ein Buchrücken, der **nicht** der
  gebaute ist.
* **E-77**, im dunklen Theme sind alle Trophäen-Beschriftungen unlesbar.

Drei davon sind behoben oder nicht nachgebaut, E-76 ist nachgebaut wie er ist.

#### Was beim Bauen überrascht hat

* **Der eigene JS-Leser konnte den Schlüssel `münchen` nicht lesen.**
  `_identifier()` in `tool/generate_curated_data.dart` nahm nur
  `[A-Za-z0-9_$]`, las bei `münchen:` das `m` und meldete „Kein `:` nach dem
  Schlüssel". Das war **laut** und nicht still, der Leser hat sich also
  richtig verhalten. Erlaubt ist jetzt `U+00C0` bis `U+024F`, also Latin-1
  und Latin Extended-A/B; alles darüber wirft weiter.
* **Zwei Einträge der Registrierung auf derselben Quelldatei.** Trophäen
  gehören `progression`, Städte gehören `collection`. Ein gemeinsamer
  Ausgabepfad hätte ein Feature das Datum des anderen importieren lassen. Die
  Datei wird deshalb zweimal gelesen. Die Nebenwirkung aus Schritt 49 ist damit
  ein drittes Mal aufgetreten, diesmal nicht in der Zahl der Dateien, sondern
  in der Zahl der Blöcke je Datei: die Ersatz-PWA im Test braucht jetzt beide.
* **Der gestrichelte Rahmen war schon einmal gebaut.** Flutter hat keinen, und
  `challenge_setup_view.dart` trug seit Schritt 34 einen privaten Maler. Der
  Leerplatz des Regals ist die zweite Stelle, Regel 8 verbietet den Zugriff auf
  die Presentation eines anderen Features, und eine zweite Abschrift wäre genau
  das Muster, an dem dieses Repository sonst zu suchen anfängt. Ausgelagert nach
  `core/widgets/dashed_border_painter.dart`, mit `BorderRadius` statt eines
  einzelnen Radius, weil ein Buchrücken ungleiche Ecken hat.
* **`aria-label` ersetzt den Inhalt, es ergänzt ihn nicht.** Ohne
  `excludeSemantics: true` lagen Titel, Zähler und Bandnummer als eigene Knoten
  unter dem Satz, und ein Screenreader hätte beides vorgelesen. Der Test hat es
  gefunden, weil die Beschriftung ein angehängtes `\n` trug.
* **Der Fehlerzustand ließ sich nur mit einem `Error` prüfen, nicht mit einer
  `Exception`.** `defaultRetry` wiederholt zehnmal über rund 38 Sekunden und
  nimmt nur `Error` und `ProviderException` aus. Mit einer `Exception` wechselt
  der Zustand zwischen Fehler und Ladekreis, und jeder Zeitgeber überlebt das
  Testende. Das ist keine Testklemme, sondern das Verhalten am Gerät, und es
  steht schon bei `factOverlayProvider`.
* **Der Ladezustand war im ersten Bild schon vorbei.** Ein `async`-Override ist
  nach dem ersten Mikrotask fertig, und `pumpWidget` führt Mikrotasks aus.
  Nötig ist ein `Completer`, der nie erfüllt wird, wie in `fact_page_test.dart`.
...
## E-57, die Team-Jagd, und eine zurückgenommene Folgerung

> **Am 03.09.2026 präzisiert, und dabei nehme ich meine eigene Folgerung zurück.** Ich hatte geschlossen, die Stationen in der Mitte müssten **aufgeteilt** werden, weil „sie sollen sich nicht begegnen" mit einer gemeinsamen Menge nicht erfüllbar sei. Das war zu scharf gefolgert. Janek: „ja man wird sich schon mal begegnen, das kann schon sein, aber die zweite Station darf halt auf keinen fall die gleiche sein und die vorletze auch nicht."
>
> **Die Regel ist damit klein und genau:** Station 1 gemeinsam, Station 9 gemeinsam, und die **zweite** und die **vorletzte** Position müssen sich zwischen den Teams unterscheiden. Dazwischen darf gekreuzt werden. Gelegentliches Begegnen ist ausdrücklich in Ordnung.
>
> **Und damit ändert sich die Bewertung des Ausgleichs.** Bei derselben Menge Stationen ist die Gesamtstrecke beider Teams von vorn herein gleich, „ähnliche Distanz" ist also trivial erfüllt. Der dreifache Resampling-Apparat ist damit nicht kaputt, sondern **überflüssig**; er löst ein Problem, das mit einer gemeinsamen Menge nicht entsteht. Was wirklich fehlt, ist die Nachbarschaftsbedingung an Position 2 und 8, und die ist eine Zeile, keine Umstellung des Datenmodells. Die wirkungslose Winkelnormalisierung bleibt ein eigener Fehler daneben.
...
> **Am 03.09.2026 von Janek beantwortet, und die Antwort hat die
> Frage gesprengt.** Janek: „Klar, dann müssen alle Antworten natürlich
> auch auf Englisch sein! ist Doch klar!!" Beim Nachmessen, wie groß das
> Übersetzen wäre, kam heraus, dass es nichts behebt: von 1911
> Rätseln sind 247 überhaupt richtig zu beantworten, und zwar auch
> auf Deutsch. Die Auswertung vergleicht Zeichenketten, und
> `expected` enthält Lösungsnotizen für Menschen. Alle Zahlen in
> **E-79**. Die Frage an Dairen ist damit hinfällig in dieser Form;
> vorher gehört eine Produktantwort her, was ein Rätsel in dieser
> App überhaupt ist.
> **Am 03.09.2026 zurückgestellt, und zwar ausdrücklich.** Janek: „zu D19
> hat Dairen gesagt man löst das ganz anders. also hier noch nichts
> machen bitte"
>
> **Damit sind die drei Wege am Ende von E-79 keine Auswahl mehr, sondern
> nur noch die Landkarte, die beim Messen entstanden ist.** Wer sie später
> liest, darf keinen davon als beschlossen lesen; der Weg kommt von
> Dairen und ist hier nicht bekannt.
>
> **Was das für den Code heißt:** an der Rätsel-Auswertung entsteht nichts,
> auch nichts Vorbereitendes. Die Schritte 28, 29, 30 und 38 bleiben
> gesperrt, und `puzzle_sheet.dart` behält seine leere Stelle. Ein
> Vertrag, der auf einen vermuteten Lösungsweg zugeschnitten ist, wäre
> teurer als keiner: er sieht wie eine Entscheidung aus und ist geraten.
>
> **Die Messung bleibt trotzdem stehen.** Sie ist der Grund, warum ein
> reines Übersetzen der Antworten nicht genügt, und genau das war die
> Annahme, unter der die Frage gestellt wurde. Was auch immer der andere
> Weg ist, er muss die 82 Zähl-Rätsel erklären, die heute gegen null
> vergleichen.
 Gehört zu E-08 und blockiert die Schritte 28, 29, 30 und 38, also die
...
> **Am 03.09.2026 an mich delegiert.** Janek: „D-20 musst du dir überlegen."
> Damit ist es der **dritte** delegierte Fall nach D-15 (→ ADR-006) und D-16
> (→ ADR-007), und wie bei jenen entsteht ein ADR und nicht nur eine
> Codeänderung: die Frage macht ADR-007 auf, und eine Nutzlast, die einen
> `HuntPlan` trägt, ist genau die Entscheidung, die dort bewusst anders
> gefallen war.
>
> **Die Frage geht damit nicht mehr an Dairen** und steht ab hier auf meiner
> Seite. Sie bleibt in diesem Abschnitt stehen, weil der dritte Block sie im
> Wortlaut verschickt hat und eine gelöschte Frage später nicht mehr auflösbar
> wäre.
...
| E-17 | **Creator-Foto.** Storage-Bucket, Policy, Moderation vor `is_approved`.  **Am 03.09.2026 entschieden, und die Antwort trennt zwei Foto-Arten, die vorher zusammen gedacht waren.** Janek: „Fotos im Creator Modus sollen einfach auf Supabase gespeichert werden, die sind schon wertvoll. Fotos die aber während der Jagd im Rahmen von Fotografiere das gemacht werden und dann einfach nur approved werden müssen brauchen aber nicht gespeichert werden." **Creator-Foto:** geht nach Supabase, braucht also den Bucket, den es laut `docs/operations/backend-inventory.md` überhaupt nicht gibt, dazu Moderation und eine Löschregel. Stufe 4 bleibt für die Bucket-Anlage. **Foto-Rätsel der Jagd:** wird **nicht** gespeichert. Kein Bucket, keine Moderation, keine Löschregel, keine Auskunftspflicht. Das Bild entsteht, wird bestätigt und ist weg. **Damit hat die Kamera drei Verwendungen mit drei verschiedenen Folgen**, und nur die dritte verlässt das Gerät: Sucher ohne Aufnahme (Damals/Heute, E-20), Aufnahme, die auf dem Gerät bleibt (Foto-Rätsel), Aufnahme, die hochgeladen wird (Creator). Die Zweckbindung muss alle drei nennen und den Unterschied deutlich machen. **Der Halbsatz „nur approved werden" berührt E-79 und wird hier nicht ausgebaut:** wie ein Foto-Rätsel bewertet wird, gehört zu dem Weg, den Dairen für D-19 vorgesehen hat, und daran entsteht nichts. | 3, Bucket-Anlage 4 | **entschieden 03.09.2026**, Bucket offen |
...
| E-20 | **Kamera-Permission** für Damals/Heute und Foto-Rätsel, mit Zweckbindung. **Am 03.09.2026 entschieden, und dabei eine Annahme des Eigentümers berichtigt.** Janek nahm an, Damals/Heute brauche gar keine Kamera: „man öffnet die VR Ansicht und dort ist dann in Blickrichtung das Bild eingepflegt … eigentlich braucht es nur Standort und Blickrichtung oder?" **Gemessen ist es anders.** `damals-heute.jsx:19-24` öffnet mit `getUserMedia` die rückseitige Kamera als **Live-Bild**, das historische Foto liegt darüber (`:53-60`), und ein Regler blendet zwischen beiden über (`dhOpacity`). Eine **Blickrichtung kommt darin überhaupt nicht vor**: kein Kompass, kein Lagesensor, keine Verankerung des Fotos an einer Himmelsrichtung. Ohne Kamera ist „Heute" schwarz. **Der Kern der Annahme trifft trotzdem zu, und er ist der wichtige Teil:** es wird **kein Foto aufgenommen**. Die Kamera ist nur Sucher, es entsteht keine Datei, nichts wird gespeichert, nichts verlässt das Gerät. Genau so gehört die Zweckbindung formuliert. **Der Rückfall ist schon in der Quelle gebaut:** bei verweigerter Erlaubnis zeigt sie das historische Foto voll deckend auf Schwarz plus den Hinweis `dh.noCamera` (`:90-93`). Die Funktion bleibt also nutzbar, nur ohne Überblendung. **Was der Eigentümer beschrieben hat, ist ein anderes, größeres Feature:** ein Foto, das an einer Himmelsrichtung hängt. Der Kompass liegt seit Schritt 14 im Repository, das wäre also baubar, ist aber neues Verhalten und keine Parität. Als eigener Wunsch festgehalten, nicht als Teil von Schritt 24.  **Am 03.09.2026 weiter entschieden, und damit fällt die Kamera für diese Ansicht ganz weg.** Janek: „nur wenn es halt cool funktioniert, dass man von genau der position wo man ist in die Vergangenheit blicken kann. das wäre halt nice. man braucht das aktuelle bild ja nicht, weil die realität siet man ja eh." **Der Kopf der Quelldatei begründet, warum die Quelle es anders macht:** „AR-Light … Kein echtes Tracking (Browser), Nutzer richtet das Handy selbst aus." Die Kamera ist dort die Krücke für ein fehlendes Tracking: man sieht im Sucher, wohin man hält, und richtet von Hand aus. **Nativ haben wir das Tracking**, der Kompass liegt seit Schritt 14 im Repository (`flutter_rotation_sensor`, Regel 24). Damit ist die Krücke überflüssig. **Der Zuschnitt für den Neubau:** das historische Foto steht im Vollbild, sobald das Handy in Richtung des Fakts zeigt, und blendet aus, wenn man sich wegdreht. Keine Kamera, keine Erlaubnis, keine Aufnahme. **Die Blickrichtung ist ableitbar und braucht kein neues Datenfeld:** die Daten tragen keine Aufnahmerichtung, wohl aber die Koordinate des Fakts, und die Peilung von der Nutzerposition zu dieser Koordinate ist genau „von hier aus dorthin blicken". Das Foto selbst kommt aus `hint_media`, wie in der Quelle. **Folge für die Zweckbindung:** die Kamera hat nur noch **zwei** Verwendungen statt drei, und Damals/Heute ist keine davon. Übrig bleiben das Foto-Rätsel (bleibt auf dem Gerät) und der Creator (wird hochgeladen), siehe E-17. **Das ist kein Nachbau mehr, sondern ein besseres Feature**, und der Eigentümer hat es ausdrücklich so gewollt. Der Regler zum Überblenden verliert damit seinen Gegenpart; ob es stattdessen ein Ein- und Ausblenden über die Peilungsabweichung gibt, ist eine Gestaltungsfrage beim Bau von Schritt 24. | 3 | **entschieden 03.09.2026** |
...
| E-57 | **Der Team-Ausgleich kann nicht wirken.** `_team_generate_orders` vergleicht die Wegstrecken über `sum(...) where id = any(v_a)` gegen `any(v_b)`; `v_a` und `v_b` sind Permutationen **derselben Menge**, also ist die Differenz immer 0, die Schwelle `<= 0.20` immer erfüllt und der dreifache Resampling-Apparat unerreichbar. Beide Teams laufen denselben Stationssatz in anderer Reihenfolge, und das ist laut Index-Kommentar gewollt; dann ist offen, was die Teams überhaupt unterscheiden soll. Dazu eine wirkungslose Winkelnormalisierung, `abs(((x-y)+pi())-pi())` kürzt sich zu `abs(x-y)`. Dieselbe Bauart wie die dreistufige Auswahl aus Schritt 33: eine Vorkehrung, die ihr Ergebnis nicht ändern kann.  **Am 02.09.2026 beantwortet, und die Antwort dreht den Befund.** Janek: dieselben Stationen für beide Teams, in **unterschiedlicher Richtung**, mit **demselben letzten Fakt**; wer dort zuerst ankommt gewinnt, aber nur bei mehr oder gleich vielen Coins. **Der Ausgleich ist damit nicht kaputt, sondern gegenstandslos**: gleich lange Wege sind gewollt und folgen aus gleichen Stationen, entschieden wird über Zeit und Coins. Er kann weg. **Neuer Befund an derselben Stelle:** die Regel „gleicher letzter Fakt“ gibt es im Code nicht. Nachgesehen: `v_b` ist im ersten Zweig `array_agg(id order by ord desc)`, also die Umkehrung von `v_a`, damit ist die letzte Station von B die **erste** von A; der zweite Zweig sortiert noch anders. Nötig ist eine als Finale festgelegte Station, die für beide die letzte ist, und nur die Stationen davor umgekehrt. **Offen bleibt eine kleine Frage mit großer Wirkung:** gegen wen und wann werden die Coins verglichen, und was gilt, wenn das schnellere Team weniger hat. Im vierten Fragenblock.  **Am 03.09.2026 entschieden.** Janek: „man startet als große gruppe trennt sich dann aber und kommt am ende wieder zusammen. Man soll aber nicht die selben Sachen gleichzeitig ansteuern … also beide Starten bei Station 01 und Enden bei Station 09. dazwischen ist Zufall, aber natürlich so, dass beide eine ähnliche Distanz laufen müssen insgesamt … sie sollen sich nicht begegnen (oder möglichst nicht begegnen) aber am Ende haben sie das gleiche Ziel." **Die Vorgabe erklärt gleichzeitig, warum der Ausgleich der Quelle nicht wirken kann.** `_team_generate_orders` gibt beiden Teams **dieselbe** Menge Stationen in anderer Reihenfolge; dann ist die Streckendifferenz von vorn herein null, und die Teams begegnen sich zwangsläufig ständig. „Sie sollen sich nicht begegnen" ist mit einer gemeinsamen Menge nicht erfüllbar. **Daraus folgt der Zuschnitt, und der ist eine Änderung am Datenmodell und nicht an einer Schwelle:** erste Station gemeinsam, letzte Station gemeinsam, und die Stationen dazwischen werden **aufgeteilt**, jedes Team bekommt seine eigenen. Erst dadurch entstehen überhaupt zwei verschiedene Wegstrecken, und erst dadurch bekommt der Ausgleich etwas zu tun. Die Winkelnormalisierung, die sich zu `abs(x-y)` kürzt, bleibt ein eigener Fehler daneben.  **Nachtrag 03.09.2026, siehe unten: die Folgerung „Mitte aufteilen" ist zurückgenommen.** | 2 | **entschieden 03.09.2026** |
...
| E-73 | **Der Reiseführer beschriftet dreißig seiner sechsunddreißig Trophäen mit dem rohen Schlüsselnamen.** Am 03.09.2026 gezählt, nicht vermutet. `screen-wallet.jsx:1050` holt die Beschriftung mit `t('trophy.' + tr.key, lang)`. In `translations.jsx` gibt es genau **elf** `trophy.*`-Schlüssel, und vier davon sind Oberflächentexte (`trophy.collection`, `.count`, `.unlock`, `.viewAll`). Übrig bleiben sechs Trophäennamen, die zu einem Eintrag in `WalletTrophies` passen; ein siebter, `trophy.firstEver`, passt zu keinem. Auf dem Bildschirm stehen damit dreißig Zeichenketten wie `meister_hist`, `münchen_first`, `weltenbummler`. **Dieselbe Fehlerklasse wie E-28 und E-63, und die Antwort liegt unmittelbar daneben:** die Namen stehen zweisprachig in den Daten (`label_de`, `label_en`), und `screen-profil.jsx:210` liest genau sie. Zwei Bildschirme, dieselben 36 Definitionen, einer funktioniert. **Behoben, ohne dass eine Textentscheidung nötig war** (`library_trophy_row.dart`), und der Test hängt an der Lücke selbst, nicht nur am Text. | 2 | **behoben 03.09.2026** |
| E-74 | **Die größte Zahl auf dem Reiseführer ist doppelt so groß wie die Wahrheit.** Am 03.09.2026 gemessen. `WalletScreen` (`screen-wallet.jsx:1811-1816`) baut die Sammelmenge mit `(collectedFacts \|\| []).forEach(id => { s.add(id); s.add(String(id)); })`. Der Grund ist echt und dokumentiert: die Kennungen sind gemischt, Supabase liefert Zahlen, selbst angelegte Fakten tragen `'local-…'`, und beide Formen in die Menge zu legen macht jede Suche robust. Zwei Zeilen später steht aber `const totalCollected = collectedSet.size` (`:1864`), und für eine Zahlkennung enthält die Menge **zwei** Einträge, `42` und `'42'`. Die Schlagzeile „N Geschichten aus deinen Städten" zeigt damit `2 × Datenbank-Fakten + lokale Fakten`. **Dieselbe App rechnet an anderer Stelle richtig:** `app.jsx:706` nimmt `collectedFacts.length` für die Erfahrungspunkte. Zwei Zahlen für dieselbe Sache, eine davon doppelt. **Kann uns nicht passieren:** `FactId` hat Wertgleichheit, und `CollectedFactsNotifier.collect` prüft auf `contains`. Nicht nachgebaut, siehe `collectedFactCountProvider`. | 2 | **nicht nachgebaut 03.09.2026** |
...
| E-78 | **Fünf Kartenkategorien landen im Reiseführer im Kapitel „Historisch", darunter Kulinarik.** Am 03.09.2026 gemessen und in `library_categories_test.dart` festgenagelt. `walletKatToKey` (`wallet-colors.jsx:88-100`) kennt sechs Ziele und fällt sonst auf `hist` zurück. Von den Kategorietexten, die `factCategoryAliases` als eigene Kartenkategorien führt, treffen deshalb **alle vierzehn** Schreibweisen von Kulinarik (`kul`), Persönlichkeiten (`pers`), Kultur (`kult`), Dunkel & Kriminell (`dark`) und Kirche & Glaube (`kirche`) auf `hist`. Ein Fakt über ein Restaurant steht im Reiseführer damit unter Geschichte. Der auffälligste Einzelfall ist `Historical Figures`: die Karte führt es ausdrücklich als `pers`, der Reiseführer macht daraus Geschichte, weil der Text mit „hist" anfängt. Zwei Tabellen, ein Text, zwei Antworten. **Eine Ausnahme ist gewollt:** `Natur` fällt auf `geo`, dafür hat die Quelle eine eigene Zeile. **Übernommen wie es ist, und das ist eine Abwägung:** sechs Kapitel sind eine gestalterische Entscheidung, zwölf daraus zu machen wäre neues Verhalten und keine Behebung. Dass ein kulinarischer Fakt unter Geschichte einsortiert wird, ist einem Leser gegenüber trotzdem falsch. **Die Antwort ist eine Inhaltsentscheidung des Eigentümers**, nicht eine Codeänderung: entweder eigene Kapitel für die fünf, oder ein Kapitel „Vermischtes", oder es bleibt. **Am 03.09.2026 nachgemessen, und es macht die Entscheidung billiger:** es gibt dreizehn `cat.*`-Schlüssel, und der Reiseführer erreicht sechs. Für drei der fünf zusammengefalteten Kategorien liegt der Name also schon zweisprachig vor (`cat.kul`, `cat.pers`, `cat.chr`), für zwei nicht (`cat.kult`, `cat.dark`). Eigene Kapitel für die fünf kosten damit höchstens zwei neue Wortlaute und keine fünf. Derselbe Befund hat in Schritt 21 die Akte geprägt, siehe `facts/presentation/fact_category_look.dart`: dort hält der Neubau bewusst die **kleinere** Tabelle der Quelle, weil sie ausschließlich Schlüssel erreicht, die es gibt.  **Am 03.09.2026 entschieden: eigene Kapitel.** Janek: „ne, das soll nicht alles unter historisch. die bekommen ihr eigenes akapitel, wäre am cleansten oder?" Damit hat der Reiseführer **elf** Kapitel statt sechs. **Kein Wortlaut nötig, und das war beim Fragen noch nicht klar:** `cat.kul`, `cat.pers` und `cat.chr` stehen zweisprachig in den erzeugten Tabellen, und für `kult` und `dark` liefert die Aliastabelle der Karte beide Sprachen mit (`Kultur`/`Art & Culture`, `Dunkel & Kriminell`/`Dark & Criminal`). Der Schlüssel `kirche` wird auf `cat.chr` abgebildet, weil das Wörterbuch ihn so führt. **`nat` bleibt bei `geo`**: dafür hat `walletKatToKey` eine eigene Zeile, das ist eine Absicht der Quelle und kein Rückfall, also elf und nicht zwölf. | 2 | **entschieden 03.09.2026** |
